...
# generate bin break vector using bin.breaks(); req arg is "bins"
vec.bin <- bin.breaks()
...
lambdas <- rep(NA, length(bin.num))
    ...
    for(i in 1:(length(vec.bin)-1)){
        ...
        # at t1 from the bin set at t0, that is growth or shrinkage or neither   
    }
    
    M <- matrix(NA, n.bin, n.bin)   # initiate projection matrix
    r <- 0.185                      # MLE for r from Shriver (2012)
    
    # populate projection martix
